...
if not all([ALGOLIA_BASE_URL, ALGOLIA_API_KEY, ALGOLIA_APP_ID]):
    logging.critical(
        "Отсутствуют необходимые переменные окружения (ALGOLIA_BASE_URL, ALGOLIA_API_KEY, ALGOLIA_APP_ID)"
    )
    exit(1)
...
if os.path.exists(ID_LIST_FILE):
    try:
        ...
        logging.error(f"Ошибка чтения {ID_LIST_FILE}: {e}")
...
async def get_contact_phone_from_html(session: ClientSession, external_id: str) -> str:
...
async def get_furnishing_from_html(session: ClientSession, external_id: str) -> bool | None:
    url = f"https://www.bayut.com/property/details-{external_id}.html"
    try:
        async with session.get(url, headers=headers, timeout=10) as response:
            if response.status != 200:
                logging.error(f"[HTML] статус {response.status} для {external_id}")
                return None
            html = await response.text()
            soup = BeautifulSoup(html, "html.parser")
        ...
        logging.error(f"[HTML] исключение для {external_id}: {e}")
        return None
...
async def cleanup_old_ids_and_db():
    ...
    try:
        ...
        logging.error(f"Ошибка при очистке ID и БД: {e}")
...
async def fetch_hits(session: ClientSession, page: int) -> dict:
...
async def fetch_detail(session: ClientSession, object_id: str) -> dict | None:
            ...
            async with async_session() as db_session:
                async with db_session.begin():
                    for hit in new_hits:
                        ...
                        rooms = "100" if rooms_raw in [None, 0] else str(rooms_raw)
                        ...
                        photo_ids = [str(photo_id) for photo_id in detail_hit.get("photoIDs", [])]
                        ...
                        try:
                            ...
                            logging.error(
                                f"Ошибка сохранения в БД для external_id={external_id}: {e}"
                            )
                        await asyncio.sleep(2)  # Уменьшен интервал для оптимизации
        except Exception as e:
            logging.error(f"Критическая ошибка в process_new_ads: {e}")
        finally:
            await session.close()

    if new_hits:
        try:
            with open(ID_LIST_FILE, "a", encoding="utf-8") as f:
                for hit in new_hits:
                    f.write(f"{hit['externalID']}\n")
            logging.info(f"Добавлено новых ID в id_list.txt: {len(new_hits)}")
        except Exception as e:
            logging.error(f"Ошибка записи в {ID_LIST_FILE}: {e}")
...
async def main_parser():
    while True:
        try:
            ...
            logging.error(f"Ошибка в main_parser: {e}")
